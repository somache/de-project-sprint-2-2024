...
--таблицы источники (откуда взять)
SELECT id, craftsman_id, craftsman_name, craftsman_address, craftsman_birthday, craftsman_email, product_id, product_name, product_description, product_type, product_price, order_id, order_created_date, order_completion_date, order_status, customer_id
...
-- таблицы хранилища (куда положить)
...
--1 собираем временную таблицу с данными из нового источника
DROP TABLE IF EXISTS tmp_sources;
CREATE TEMP TABLE tmp_sources AS 
(
select 
 o.order_id -- идентификатор заказа;
,o.order_created_date --дата создания заказа;
,o.order_completion_date -- дата выполнения заказа;
,o.order_status -- статус заказа;
,o.craftsman_id -- идентификатор мастера;
,o.craftsman_name -- имя мастера;
,o.craftsman_address -- адрес мастера;
,o.craftsman_birthday -- день рождения мастера;
,o.craftsman_email -- электронная почта мастера;
,o.product_id -- идентификатор товара;
,o.product_name -- наименование товара;--
,o.product_description -- описание товара;
,o.product_type -- тип товара;
,o.product_price -- цена товара;
...
--2.1 обновление существующих записей и добавление новых в dwh.d_craftsmans
...
--2.2 обновление существующих записей и добавление новых в dwh.d_customer 
...
WHEN NOT MATCHED THEN
 ...
 --2.3 обновление существующих записей и добавление новых в dwh.d_products 
...
--2.4 создание таблицы tmp_sources_fact 
...
--2.5 обновление существующих записей и добавление новых в dwh.f_order 
...
WHEN NOT MATCHED THEN
 ...
 --3 создание витрины данных dwh.customer_report_datamart
...
CREATE TABLE IF NOT EXISTS dwh.customer_report_datamart (
    ...
    customer_money NUMERIC(15,2) NOT NULL, -- сумма денег, которую потратил заказчик
...
--4 доп. таблица с датой загрузки для инкрементального обновления
...
SELECT  
			...
			FROM (
				SELECT 	-- в этой выборке объединяем три внутренние выборки по расчёту столбцов витрины и применяем оконную функцию, чтобы определить самую популярную категорию товаров и идентификатор самого популярного мастера
						*,
						RANK() OVER(PARTITION BY T2.customer_id ORDER BY count_product DESC) AS rank_count_product 
						,first_value(craftsman) OVER(PARTITION BY T2.customer_id ORDER BY count_order_id DESC) as top_craftsman_id
						FROM ( 
							SELECT -- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, кроме столбца с самой популярной категорией товаров у мастера. Для этого столбца сделаем отдельную выборку с другой группировкой и выполним join
								...
								SUM(T1.product_price) as customer_money, -- сумма денег, которую потратил заказчик
								...
								PERCENTILE_CONT(0.5) WITHIN GROUP(ORDER BY diff_order_date) as median_time_order_completed, -- медианное время в днях от момента создания заказа до его завершения за месяц
								...
								INNER JOIN (
									SELECT 	-- Самая популярная категория товаров.
								...
								INNER JOIN (
									SELECT 	-- идентификатор самого популярного мастера ручной работы у заказчика. Если заказчик сделал одинаковое количество заказов у нескольких мастеров, возьмите любого;
											dd.customer_id AS customer_id_for_top_craftsman_id, 
											COUNT(dd.order_id) AS count_order_id
											,dd.craftsman_id as craftsman
											FROM dwh_delta AS dd
												GROUP BY dd.customer_id, dd.craftsman_id
													ORDER BY count_order_id desc) AS T33 ON T2.customer_id = T33.customer_id_for_top_craftsman_id					
						...
						*,
						...
						FROM (
							SELECT -- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, кроме столбца с самой популярной категорией товаров у мастера. Для этого столбца сделаем отдельную выборку с другой группировкой и выполним join
								...
								FROM (
									SELECT 	-- в этой выборке достаём из DWH обновлённые или новые данные по заказчикам, которые уже присутствуют в витрине
			                               dcs.customer_id AS customer_id,
			                               dcs.customer_name AS customer_name,
			                               dcs.customer_address AS customer_address,
			                               dcs.customer_birthday AS customer_birthday,
			                              ...
			                              FROM dwh.f_order fo 
				                                ...
				                                INNER JOIN dwh.d_customer dcs ON fo.customer_id = dcs.customer_id 
				                                INNER JOIN dwh.d_product dp ON fo.product_id = dp.product_id 
												INNER JOIN dwh_update_delta ud ON fo.customer_id = ud.exist_customer_id
								) AS T1
									GROUP BY T1.customer_id, T1.customer_name, T1.customer_address, T1.customer_birthday, T1.customer_email, T1.report_period
							) AS T2 
								INNER JOIN (
...
update_delta AS (
	...
	FROM (
		SELECT 
			...
			count_order_done,
			count_order_not_done,
			report_period from dwh_delta_update_result) AS updates
	WHERE customer_report_datamart.customer_id = updates.customer_id
),
...
INSERT INTO dwh.load_dates_customer_report_datamart (
	...
	select 
    GREATEST(COALESCE(MAX(craftsman_load_dttm), MAX(customers_load_dttm), MAX(products_load_dttm), NOW()))
